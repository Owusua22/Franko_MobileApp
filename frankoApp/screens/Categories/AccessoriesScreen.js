  ...
  useFocusEffect(
    React.useCallback(() => {
      const checkScrollRestore = async () => {
        try {
          const savedScrollPosition = await AsyncStorage.getItem("AccessoriesScreenScrollPosition");
  ...
  const handleProductPress = async (productId) => {
    try {
      ...
      if (flatListRef.current) {
        await AsyncStorage.setItem("AccessoriesScreenScrollPosition", currentScrollOffset.toString());
      }
    } catch (error) {
  ...
  const handleShare = async () => {
    try {
      const result = await Share.share({
        message: 'Check out these amazing Accessories at great prices!',
        url: 'https://your-app-link.com/Accessories', // Replace with your actual app link
