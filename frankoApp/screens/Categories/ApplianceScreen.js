  ...
  useFocusEffect(
    React.useCallback(() => {
      const checkScrollRestore = async () => {
        try {
          const savedScrollPosition = await AsyncStorage.getItem("TelevisionScreenScrollPosition");
  ...
  const handleProductPress = async (productId) => {
    try {
      ...
      if (flatListRef.current) {
        await AsyncStorage.setItem("TelevisionScreenScrollPosition", currentScrollOffset.toString());
      }
    } catch (error) {
  ...
  const handleShare = async () => {
    try {
      const result = await Share.share({
        message: 'Check out these amazing Appliances at great prices!',
