  ...
  useFocusEffect(
    React.useCallback(() => {
      const checkScrollRestore = async () => {
        try {
          const savedScrollPosition = await AsyncStorage.getItem("FridgeScreenScrollPosition");
  ...
  const handleProductPress = async (productId) => {
    try {
      ...
      if (flatListRef.current) {
        await AsyncStorage.setItem("FridgeScreenScrollPosition", currentScrollOffset.toString());
      }
    } catch (error) {
  ...
  const handleShare = async () => {
    try {
      const result = await Share.share({
        ...
        url: 'https://your-app-link.com/Fridge', // Replace with your actual app link
